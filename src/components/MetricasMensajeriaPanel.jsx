...
import {
    BarChart3, RefreshCw, Calendar, MessageSquareText, FileText, Send, Phone
} from 'lucide-react';
...
export default function MetricasMensajeriaPanel({ addToast }) {
    ...
    const metrics = useMemo(() => {
        ...
        // Convert daily to array, sorted by date logic is a bit tricky with string dd/mm, but ok for chart
        // Better sort by actual date
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '24px', marginBottom: '24px' }}>
                                    ...
                                    <div style={{ height: '300px', width: '100%' }}>
                                        <ResponsiveContainer>
                                            <BarChart data={metrics.dailyChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                                ...
                                                {metrics.typeBreakdown.slice(0, 5).map((type, i) => (
                                                    <Bar key={type.name} dataKey={type.name} stackId="a" fill={COLORS[i % COLORS.length]} radius={i === metrics.typeBreakdown.slice(0, 5).length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]} />
                                                ))}
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </div>
                            ...
                            <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '24px' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                    {metrics.typeBreakdown.length > 0 ? metrics.typeBreakdown.map((item, i) => (
