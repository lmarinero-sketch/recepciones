...
export function subscribeToMessages(phone, callback) {
    ...
    const channelName = `chat-${normalized}-${Date.now()}`;
    // Remove any stale channel with the same prefix
    try {
        supabase.getChannels().forEach(ch => {
            if (ch.topic?.startsWith(`realtime:chat-${normalized}`)) {
...
export function subscribeToAllIncoming(callback) {
    // Remove any stale 'all-incoming' channels from previous HMR cycles
    try {
        supabase.getChannels().forEach(ch => {
            if (ch.topic?.includes('all-incoming')) {
                supabase.removeChannel(ch);
            }
        });
    } catch(e) { /* ignore */ }

    const channelName = `all-incoming-${Date.now()}`;
    const channel = supabase
        .channel(channelName)
        .on(
            'postgres_changes',
